...
const Auth = () => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    setIsLoading(false);
    if (error) {
      const errorMessage = error.message.includes("already registered") ? "该邮箱已注册，请直接登录" : error.message;
      toast({
        title: "注册失败",
  ...
  return <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        {/* Logo */}
        <div className="gap-3 mb-8 flex items-center justify-start">
              ...
              <TabsContent value="signup">
                <form onSubmit={handleSignup} className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input id="signup-name" type="text" placeholder="您的姓名" value={signupName} onChange={e => setSignupName(e.target.value)} required />
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="signup-password" type="password" placeholder="至少6位字符" value={signupPassword} onChange={e => setSignupPassword(e.target.value)} required />
                  </div>
                  <Button type="submit" className="w-full" disabled={isLoading}>
                    {isLoading ? <>
                        <Loader2 className="mr-2 h-4 w-4 animate-spin" />
